...
async function handleRequest(request) {
    ...
    // User-Agent başlığını ayarlıyoruz
    const version = '1.0';  // Versiyon numarasını değiştirebilirsiniz
    const userAgent = `Televizio/${version}`;

    // Ortam değişkeni ile GH_TOKEN'ı kullanıyoruz
    const headers = new Headers();
    headers.set('User-Agent', userAgent);
    headers.set('Authorization', `token ${GH_TOKEN}`);  // GH_TOKEN'ı burada kullanıyoruz

    // Kullanıcı verisini almak için GitHub API'yi çağırıyoruz
...
http://iptv-info.local/expire`;
    ...
    const m3uResponse = await fetch(m3uLink, { headers });
